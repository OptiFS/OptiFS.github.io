...
const blogs = [
    ...
    },
    {
        ...
        author: "Niall Ryan",
        imageUrl: "niall.jpg",
        content: "Today I looked into what syscall operations we'd have to implement to allow basic functionalities of files in our FUSE filesystem!<br><br>Really exciting progress today, I built my own persistent inode structure within libfuse. It was pretty fascinating learning how to define how a filesystem would store and represent its files. In particular, I focused on implementing these essential file operations;<br><ul><li><strong>Open</strong>: The foundation for accessing a file's contents.</li><li><strong>Read</strong>: How you retrieve data from a file.</li><li><strong>Lookup</strong>: Verifying that a file or directory exists within the filesystem.</li><li><strong>Getattr</strong>: Getting metadata about a file or directroy (size, permissions , etc...).</li></ul><br><br>Another key insight was the need for a <strong>separate root node type</strong>. This serves as the crucial mountpoint for my system and required slightly different behaviours and attributes than a typical node. It also proves useful as somewhat of a gateway point in our code from our virtual FUSE filesystem and the actual underlying filesystem. It essentially provides as an anchorpoint in our loopback filesystem design - and figuring this out, I think, is a big step.<br><br>Finally, I tried experimenting further with NFS and FUSE today, but I wasn't able to make any progress. It's difficult as we're just not able to tell what filesystem calls need to be implemented for NFS to work - and there's seemingly no way to figure out. I can't even find anything online about it. I believe NFS just assumes regular filesystem behaviour, so we'll have to keep fleshing out our implementation until it seems to be able to work nicely with NFS.<br><br>Finally, I also spent some time today trying to dockerise our FUSE virtual filesystem. It, however, similarly to NFS, just refuses to comply with Docker. So for now, I've just decided I'm going to leave the dockerisation until a later date, if at all!",
    },
]
